...
class UsageStatsNameFeature(sensorFeature):
  name = 'usageStatsName'
  def process(self, sensorData):
    networkSetup.lastFeatures['usageStatsName'] = stringToInt(sensorData['usageStats'][0]['name'])
...
#GENERAL
generalSensor = sensor('general')
addSensor(generalSensor)

class TimeHourFeature(sensorFeature):
  name = 'hour'
  def process(self, sensorData):
    hour, minute = stampToTimeUnits(sensorData['time'])
    networkSetup.lastFeatures['hour'] = hour
timeHourFeature = TimeHourFeature()
addFeature(timeHourFeature, generalSensor)

class TimeMinuteFeature(sensorFeature):
  name = 'minute'
  def process(self, sensorData):
    hour, minute = stampToTimeUnits(sensorData['time'])
    networkSetup.lastFeatures['minute'] = minute
TimeMinuteFeature = TimeMinuteFeature()
addFeature(TimeMinuteFeature, generalSensor)
...
def input_fn(data_set, isAuthentic = 1):
  ...
  networkSetup.numSensorObjects = num_examples
...
def predict(filepath):
  ...
  predictions = list(itertools.islice(y, networkSetup.numSensorObjects))
